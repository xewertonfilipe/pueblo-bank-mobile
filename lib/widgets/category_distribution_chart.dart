...
class CategoryDistributionChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (total == 0) return const Card(child: SizedBox(height: 180, child: Center(child: Text('Ainda nao ha dados para exibir.'))));
    return SizedBox(
      height: 220,
      child: Card(
        child: PieChart(PieChartData(sectionsSpace: 3, centerSpaceRadius: 38, sections: [
          PieChartSectionData(value: provider.summaryDeposits, color: Colors.green, title: 'Depositos', radius: 72, titleStyle: const TextStyle(color: Colors.white, fontSize: 12)),
